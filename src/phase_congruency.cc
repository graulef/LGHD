...
void phase_congruency::compute() {
    ...
    for (size_t e = 0; e < m_filter_bank->get_num_elevations(); ++e) {
        ...
        for (size_t a = 0; a < m_filter_bank->get_num_azimuths(e); ++a) {
            ...
            write_image<float, 3>(filename,
                array2image<float, 3>(directional_pc_map, m_sizes));
    ...
    write_image<TVector, 3>(filename,
        array2image<TVector, 3>(directional_pc_max_map, m_sizes));
    ...
    write_image<float, 3>(filename, array2image<float, 3>(pc_map, m_sizes));
    ...
    for (size_t d = 0; d < 3; ++d) {
        sprintf(filename, "%s_eigenvalues_%u.nii", m_filename_prefix.c_str(),
                d);
        write_image<float, 3>(filename,
            array2image<float, 3>(moments_eigenvalues_maps[d], m_sizes));

        sprintf(filename, "%s_eigenvectors_%u.nii", m_filename_prefix.c_str(),
                d);
        write_image<TVector, 3>(filename,
            array2image<TVector, 3>(moments_eigenvectors_maps[d], m_sizes));
    }
