...
log_gabor_filter_bank::~log_gabor_filter_bank() {
    delete[] m_num_azimuths_per_elevation;
}


std::ostream& log_gabor_filter_bank::print(std::ostream &os) const {
    os << "{"
...
float* log_gabor_filter_bank::get_filter(size_t scale, size_t azimuth, size_t elevation) {
    assert(elevation < m_num_elevations);
    assert(azimuth < m_num_azimuths_per_elevation[elevation]);
    assert(scale < m_num_scales);
...
log_gabor_filter_bank* log_gabor_filter_bank::read_parameters(std::string filename) {
    assert(!filename.empty());
    assert(filename.find_last_of(".bof") != std::string::npos);
...
void log_gabor_filter_bank::write_parameters(log_gabor_filter_bank &bof) {
...
float* log_gabor_filter_bank::read_filter(size_t scale, size_t azimuth, size_t elevation) {
    char filename_suffix[64];
    ...
    float *filter = image2array<float, 3>(
                        read_image<float, 3>(filename));

    return filter;
}


void log_gabor_filter_bank::write_filter(float *filter, size_t scale, size_t azimuth, size_t elevation) {
    char filename_suffix[64];
    ...
    write_image<float, 3>(
        filename, array2image<float, 3>(filter, m_sizes));
}


// ----------------------------------------------------------------------------

std::ostream& operator<<(std::ostream &os, const log_gabor_filter_bank &lgfb) {
